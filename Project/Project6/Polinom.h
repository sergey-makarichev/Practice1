...
{
	TNode<unsigned int, float>* first = monoms->pFirst;////
	TNode<unsigned int, float>* second = polynom.monoms->pFirst;
...
Polinom Polinom::operator+(const TNode<unsigned int, float>& monom)
{
	...
	{
		TNode<unsigned int, float> CurrentMon(monoms->GetCurrent()->key, monoms->GetCurrent()->pData);
		if (CurrentMon > monom)
